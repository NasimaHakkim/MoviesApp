...
import learn.android.retrofitgithub.`interface`.Movies.UpcomingMovies.UpcomingMoviesClient
 ...
 */
class TopRatedMoviesPresenter(var mRecyclerView: RecyclerView, var context: Context){
    private var mAdapter: TopRatedMoviesAdapter? = null
                ...
                .subscribe(object : Observer<MovieData> {
                    override fun onNext(t: MovieData) {
                        for(i in t.results!!.indices) {
                            moviesList = t
                            mAdapter = TopRatedMoviesAdapter(moviesList)
